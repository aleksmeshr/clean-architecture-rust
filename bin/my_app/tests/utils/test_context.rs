use crate::integration_tests::fixtures::dog_facts_fixtures;
use animal_facts_repositories::postgres::db_connection::DbConnection;
use diesel::RunQueryDsl;
use dog_facts_fixtures::execute_imports;
...
impl TestContextPostgreSQL {
    ...
    pub fn new(base_url: &str, db_name: &str) -> Self {
        ...
        let conn_postgres_db = db_connection_postgres_db
            .get_pool()
            .get()
        ...
        query
            .execute(&conn_postgres_db)
            .unwrap_or_else(|_| panic!("couldn't create database {}", db_name));

        // connect to the "test" db created above
        let db_connection_test_db = DbConnection { db_name: db_name.to_string() };
        let conn_test_db =
            db_connection_test_db.get_pool().get().expect("couldn't get db connection from pool");

        // create data model
        embedded_migrations::run_with_output(&conn_test_db, &mut std::io::stdout())
            .expect("couldn't run migration");

        // insert fixtures
        execute_imports(&db_connection_test_db);

        Self { base_url: base_url.to_string(), db_name: db_name.to_string() }
    }
}
...
impl Drop for TestContextPostgreSQL {
    fn drop(&mut self) {
        ...
        query
            .execute(&conn_postgres_db)
            .unwrap_or_else(|_| panic!("couldn't drop test database {}", self.db_name));
    }
}
