use animal_facts_api::presentation::models::dog_fact_models::DogFactPayload;
...
async fn test_should_return_multiple_results() {
    ...
    let content_json = response.json::<Vec<DogFactPayload>>().await.unwrap();
...
async fn test_should_return_one_results_only() {
    ...
    let content_json = response.json::<DogFactPayload>().await.unwrap();
