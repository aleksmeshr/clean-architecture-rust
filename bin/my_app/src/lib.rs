...
pub fn serve(listener: TcpListener, db_host: &str, db_name: &str) -> Result<Server, std::io::Error> {
    env::set_var("RUST_BACKTRACE", "1");
    env::set_var("RUST_LOG", "actix_web=debug");

    env_logger::try_init();

    let db_connection = DbConnection { database_host: db_host.to_string(), db_name: db_name.to_string() };
