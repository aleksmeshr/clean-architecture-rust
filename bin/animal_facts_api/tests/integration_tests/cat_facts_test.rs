use animal_facts_api::presentation::models::cat_fact_models::CatFactPayload;
...
async fn test_should_return_multiple_results() {
    ...
    let content_json = response.json::<Vec<CatFactPayload>>().await.unwrap();
...
async fn test_should_return_one_results_only() {
    ...
    let content_json = response.json::<CatFactPayload>().await.unwrap();
